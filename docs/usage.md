...
Download the [Apple Silicon CLI archive](https://github.com/dees91/agent-skill-manager/releases/download/v0.8.0/skill-manager-cli-0.8.0-macos-arm64.tar.gz)
and [SHA256SUMS.txt](https://github.com/dees91/agent-skill-manager/releases/download/v0.8.0/SHA256SUMS.txt)
from the `v0.8.0` preview into your Downloads folder. In Terminal, run:

```bash
cd "$HOME/Downloads"
grep 'skill-manager-cli-0.8.0-macos-arm64.tar.gz$' SHA256SUMS.txt \
  | shasum -a 256 -c -
```
...
The expected output is `skill-manager 0.8.0`. To keep the command available in
...
and does not appear in the `v0.8.0` public preview.
...
The current source version is `0.8.0`. It is a public preview, not a stable
release.
