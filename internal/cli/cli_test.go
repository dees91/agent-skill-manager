...
func TestRunInstallLocalPathAndListClassification(t *testing.T) {
	...
	for _, want := range []string{"install local source:", "group: sample-pack", "created 4 symlink(s)", "source remains in place:"} {
		if !strings.Contains(stdout.String(), want) {
			t.Fatalf("stdout = %q, want %q", stdout.String(), want)
		}
	}
...
func TestRunListPrintsSkillRows(t *testing.T) {
	...
	output := stdout.String()
	for _, want := range []string{"Skill", "Claude", "Codex", "Muse", "Grok", "Source", "active-claude", "ON", "imagegen", "RO", "off-skill", "OFF", "conflict-skill", "CONFLICT"} {
		if !strings.Contains(output, want) {
...
func TestRunListJSONReturnsVersionedPathFreeInventory(t *testing.T) {
	...
	for _, skill := range output.Skills {
		byName[skill.Name] = struct {
			...
			GrokState        string
			GrokToggleable   bool
		}{skill.Tools.Claude.State, skill.Tools.Claude.Toggleable, skill.Tools.Codex.State, skill.Tools.Codex.Toggleable, skill.Tools.Muse.State, skill.Tools.Muse.Toggleable, skill.Tools.Grok.State, skill.Tools.Grok.Toggleable}
	}
	if got := byName["off-skill"]; got.ClaudeState != "off" || !got.ClaudeToggleable || got.CodexState != "missing" || got.MuseState != "missing" || got.MuseToggleable || got.GrokState != "missing" || got.GrokToggleable {
...
func TestRunGroupsSummarizesRepoSkillsCLIAndDisabledGroups(t *testing.T) {
  ...
  "skills": {
	...
	output := stdout.String()
	for _, want := range []string{"Group", "Rows", "Claude", "Codex", "Muse", "Grok", "Sources"} {
		if !strings.Contains(output, want) {
			t.Fatalf("groups output = %q, want %q", output, want)
		}
	}
	...
	assertGroupRow(t, groupRows, "android/skills", "1", "ON:1 OFF:0 CONFLICT:0 RO:0", "ON:0 OFF:0 CONFLICT:0 RO:0", "ON:0 OFF:0 CONFLICT:0 RO:0", "ON:0 OFF:0 CONFLICT:0 RO:0", "symlink repo")
	assertGroupRow(t, groupRows, "Skills CLI", "1", "ON:0 OFF:0 CONFLICT:0 RO:0", "ON:1 OFF:0 CONFLICT:0 RO:0", "ON:0 OFF:0 CONFLICT:0 RO:0", "ON:0 OFF:0 CONFLICT:0 RO:0", "Skills CLI")
	assertGroupRow(t, groupRows, "stored/custom", "1", "ON:0 OFF:1 CONFLICT:0 RO:0", "ON:0 OFF:0 CONFLICT:0 RO:0", "ON:0 OFF:0 CONFLICT:0 RO:0", "ON:0 OFF:0 CONFLICT:0 RO:0", "local")
	afterState := readFile(t, p.StateFile)
	if string(afterState) != string(beforeState) {
		t.Fatalf("state changed during groups\nafter=%s\nbefore=%s", afterState, beforeState)
	}
...
func assertGroupRow(t *testing.T, rows map[string]string, group, rowCount, claudeCounts, codexCounts, museCounts, grokCounts, source string) {
	t.Helper()
	row, ok := rows[group]
	if !ok {
	...
	if got := strings.Join(fields[13:17], " "); got != grokCounts {
		t.Fatalf("group row %q Grok counts = %q, want %q", row, got, grokCounts)
	}
