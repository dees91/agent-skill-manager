...
func TestBatchToggleOneSidedRowOnlyStagesExistingCell(t *testing.T) {
	...
	if result.changed != 1 || result.skippedMissing != 3 {
		t.Fatalf("result = %#v, want one changed and three missing skips", result)
	}
	...
	if _, ok := model.pending[pendingKey{tool: domain.ToolGrok, skillName: "one-sided"}]; ok {
		t.Fatalf("grok pending exists for missing cell: %#v", model.pending)
	}
}
...
func TestGroupToggleUsesAllLoadedRowsForSelectedGroup(t *testing.T) {
	...
	for _, want := range []string{"Group android/skills", "2 pending changes updated", "Skipped 6 missing"} {
...
func TestGroupToggleNoopMessageIncludesSkippedReasons(t *testing.T) {
	...
	for _, want := range []string{"Group local: no applicable cells.", "Skipped 1 conflict", "1 read-only", "6 missing"} {
		if !strings.Contains(model.message, want) {
			t.Fatalf("message = %q, want %q", model.message, want)
		}
	}
...
func TestAllVisibleToggleHandlesMixedVisibleScope(t *testing.T) {
	...
	for _, want := range []string{"All visible rows", "1 pending change updated", "Skipped 1 conflict", "1 read-only", "14 missing"} {
		if !strings.Contains(model.message, want) {
			t.Fatalf("message = %q, want %q", model.message, want)
		}
	}
...
func TestAllVisibleToggleNoopsForEmptyAndReadOnlyOnlyRows(t *testing.T) {
	...
	model = updateModel(t, model, tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{'A'}})
	for _, want := range []string{"All visible rows: no applicable cells.", "Skipped 1 read-only", "3 missing"} {
		if !strings.Contains(model.message, want) {
			t.Fatalf("message = %q, want %q", model.message, want)
		}
	}
