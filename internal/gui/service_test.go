...
func TestToggleGroupAndApplyUsePendingSession(t *testing.T) {
	...
	if len(action.Pending) != 2 || service.PendingCount() != 2 {
		t.Fatalf("action = %#v", action)
	}
	if !action.ContextBudgets.Claude.ProjectionChanged || !action.ContextBudgets.Codex.ProjectionChanged {
	...
	if result.Failure != nil || len(result.Completed) != 2 || len(result.Snapshot.Pending) != 0 {
		t.Fatalf("apply = %#v", result)
	}
	if result.Snapshot.ContextBudgets.Claude.ProjectionChanged || result.Snapshot.ContextBudgets.Codex.ProjectionChanged {
...
func TestToggleBothReportsReadOnlyAndMissingSkips(t *testing.T) {
	...
	if result.Counts.SkippedReadOnly != 1 || result.Counts.SkippedMissing != 1 || len(result.Pending) != 0 {
