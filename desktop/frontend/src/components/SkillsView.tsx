  ...
  return (
    <div className="skills-table-wrap">
      <table className="skills-table">
        <thead><tr><th>Skill</th><th>Claude</th><th>Codex</th><th><span className="sr-only">Actions</span></th></tr></thead>
...
function cellForTool(row: SkillRow, tool: string) {
  return tool === 'claude' ? row.claude : tool === 'codex' ? row.codex : undefined
}

function rowSources(row: SkillRow) {
  return unique([row.source, row.claude?.source, row.codex?.source].filter((value): value is string => Boolean(value)))
}
